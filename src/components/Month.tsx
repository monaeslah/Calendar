        ...
        <div key={index} className="dayInThisMonth">
          ...
          {date ? (
            <Day
              ...
              selectedDate={selectedDay.startDate ? format(selectedDay.startDate, "yyyy-MM-dd") : ""}
              selectedDate2={selectedDay.endDate ? format(selectedDay.endDate, "yyyy-MM-dd") : ""}
              hoveredDate={hoveredDate}
              min={"2024-06-01"}
              max={"2024-12-31"}
              setHoverDate={setHoveredDate}
              setSelectedDate={(date) => setSelectedDay({ ...selectedDay, startDate: new Date(date) })}
              setSelectedDate2={(date) => setSelectedDay({ ...selectedDay, endDate: new Date(date) })}
              activeInput={1}
